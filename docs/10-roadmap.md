...
| ~~**P3-A8**~~ | ✅ **DONE 26 Sep**, unplanned and late. **The signers now read the transaction themselves.** `/signer/packages` used to hand over a sighash and the randomizers; a signer signed what it was given. So a compromised coordinator could serve the sighash of a *different* transaction spending the same vault, collect a valid threshold signature, and move the funds — every share verifying, aggregation succeeding, no signer able to tell. P3-A7's binding check does not help, because it runs on the coordinator. Reading moved to `quorum_core::transaction`, shared by both sides; a signer derives the sighash and the randomizers from the transaction, checks every action's `rk` against **its own** group key, and refuses on any disagreement. Plus the consent gate F2 always specified, and a request whose quorum has become unreachable now closes instead of sitting PENDING. | A | — |
...
| **P4-4** | Finalise submission: text, README, and the **security-posture review** — every external claim checked against [06-risk-register.md](06-risk-register.md) R6. **Draft rewritten 25 Sep and claim-checked by Dev A** — both txids in, the ZIP-2005/v6-anchor conflation corrected, the orchard#475 quantum-recoverability caveat added, the simulated-UI caveat added. Three `<!-- DEV B -->` blocks remain: traction (§10), business model (§11), and the video/demo links. README Status table already corrected in P4-1. What is left is Dev B's edit pass and one named person re-reading against R6. | B | 0.5d |
